...
export function WebGLShader({ className = "fixed top-0 left-0 w-full h-full block pointer-events-none" }: { className?: string }) {
  ...
  useEffect(() => {
    ...
    const initScene = () => {
      ...
      refs.renderer.setClearColor(new THREE.Color(0x000000), 0); // make background transparent so UI colors shine through!
    ...
    const handleResize = () => {
      ...
      // Size the canvas to its parent container for a localized banner effect
    ...
    // Initial delay resize to ensure parents are computed
