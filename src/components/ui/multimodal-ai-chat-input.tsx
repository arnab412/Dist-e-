...
function PureSuggestedActions({
  ...
  const suggestedActions = [
    {
      title: 'How can I convert',
      label: 'images to PDF?',
      action: 'How can I convert images to PDF?',
    },
    {
      title: 'Is there a limit',
      label: 'on PDF file size?',
      action: 'Is there a limit on PDF file size?',
    },
    {
      title: 'How to remove password',
      label: 'from PDF file?',
      action: 'How to remove password from PDF file?',
    },
    {
      title: 'Can you help me merge',
      label: 'multiple PDFs into one?',
      action: 'Can you help me merge multiple PDFs into one?',
    },
  ];
    ...
    >
      ...
      {suggestedActions.map((suggestedAction, index) => (
        ...
        >
          ...
          >
            <span className="font-medium font-bn text-sm line-clamp-1">{suggestedAction.title}</span>
            <span className="text-gray-500 font-bn text-xs line-clamp-1 pl-0">
...
const PreviewAttachment = ({
  ...
  return (
    <div data-testid="input-attachment-preview" className="flex flex-col gap-1">
      <div className="w-16 h-12 aspect-video bg-gray-200 rounded-md relative flex flex-col items-center justify-center overflow-hidden border border-gray-300">
        {contentType?.startsWith('image/') && url ? (
          ...
          <div className="flex items-center justify-center text-[10px] text-gray-600 text-center p-1 font-bn">
      ...
      <div className="text-[10px] text-gray-600 max-w-16 truncate font-bn">
        {name}
      </div>
    </div>
  );
...
function PureAttachmentsButton({
  ...
  return (
    <Button
      ...
      className="rounded-md rounded-bl-lg w-8 h-8 p-0 border border-gray-300 hover:bg-gray-200"
      onClick={(event) => {
...
function PureStopButton({ onStop }: { onStop: () => void }) {
  return (
    <Button
      ...
      className="rounded-full w-8 h-8 p-0 border border-black text-white" // Added text-white
      onClick={(event) => {
        event.preventDefault();
        onStop();
      }}
...
function PureMultimodalInput({
  ...
  return (
    <div className={cn("relative w-full flex flex-col gap-2", className)}>
      ...
      <div className="flex flex-col gap-2 border border-gray-300 dark:border-gray-700 bg-white dark:bg-slate-800 rounded-lg p-2 focus-within:ring-2 focus-within:ring-gray-600 focus-within:ring-offset-2">
          ...
          <div className="flex flex-row gap-2 flex-wrap">
            {attachments.map((attachment, index) => (
              <div key={index} className="relative group">
                ...
                <button
                  type="button"
                  className="absolute -top-1.5 -right-1.5 bg-gray-100 rounded-full p-0.5 border border-gray-300 hidden group-hover:flex"
        ...
        <Textarea
          ...
          className="border-none focus-visible:ring-0 focus-visible:ring-offset-0 min-h-[40px] h-[40px] resize-none px-2 py-2 shadow-none bg-transparent font-bn text-sm dark:text-white placeholder:text-gray-400 dark:placeholder:text-gray-500 rounded-none leading-relaxed"
          onKeyDown={(e) => {
            if (e.key === 'Enter' && !e.shiftKey) {
              e.preventDefault();
              if (input.trim() || attachments.length > 0) submitForm();
            }
          }}
        />
        
        <div className="flex flex-row items-end justify-between mt-0 px-2 pb-1">
